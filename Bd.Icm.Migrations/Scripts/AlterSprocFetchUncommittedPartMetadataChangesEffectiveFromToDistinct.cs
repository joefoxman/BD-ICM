...
{
    [Migration(201604200859)]
    public class AlterSprocFetchUncommittedPartMetadataChangesEffectiveFromToDistinct : Migration
    {
        public override void Up()
        {
            Execute.Sql(@"ALTER PROCEDURE [dbo].[spFetchUncommittedMetadataChanges](
	@InstrumentId int
)
AS
BEGIN
	WITH Metadata(PartMetadataId, PartId, Name, [Description], 
		...
		[MetaKey], [MetaValue], [InstrumentCommitId], [ModificationType], [RowVersion], 
	...
	(
		SELECT 
			...
			PM1.[MetaKey], PM1.[MetaValue], PM1.[InstrumentCommitId], PM1.[ModificationType], PM1.[RowVersion], 
			PM1.[EffectiveFrom], 
			PM1.[EffectiveTo], 
			0 AS [Level]
		FROM dbo.Part P1
		...
		SELECT 
			...
			PM2.[MetaKey], PM2.[MetaValue], PM2.[InstrumentCommitId], PM2.[ModificationType], PM2.[RowVersion], 
			PM2.[EffectiveFrom], 
			PM2.[EffectiveTo], 
			[Level]+1
		FROM Part AS P2
