...
(function () {
    ...
    fcsaNumberModule.directive('fcsaNumber', [
        'fcsaNumberConfig', function (fcsaNumberConfig) {
            ...
            makeIsValid = function (modelCtrl, options) {
                var validations = [];
                if (options.maxDecimals != null) {
                    validations.push({ key: "maxDecimals", validator: makeMaxDecimals(options.maxDecimals) });
                };
                if (options.max != null) {
                    validations.push({
                        key: "max",
                        validator: makeMaxNumber(options.max)
                    });
                }
                if (options.min != null) {
                    validations.push({
                        key: "min",
                        validator: makeMinNumber(options.min)
                    });
                }
                if (options.maxDigits != null) {
                    validations.push({
                        key: "maxDigits",
                        validator: makeMaxDigits(options.maxDigits)
                    });
                }
                return function (val) {
                    ...
                    for (i = _i = 0, _ref = validations.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
                        if (!validations[i].validator(val, number)) {
                            modelCtrl.$setValidity(validations[i].key, false);
                            return false;
                        }
                        modelCtrl.$setValidity(validations[i].key, true);
                    }
                    return true;
                };
            };
            ...
            return {
                ...
                link: function (scope, elem, attrs, ngModelCtrl) {
                    ...
                    isValid = makeIsValid(ngModelCtrl, options);
